    ...
    for perm in permutations(node_names):
        ...
        for i in range(len(current_route) - 1):
            ...
            if current_load > vehicle_capacity:
                ...
                break
            try:
                path_length = nx.dijkstra_path_length(G, current_route[i], current_route[i+1])
...
if __name__ == "__main__":
    ...
    best_route, best_distance = evalCVRP_Dijkstra()
    end_time = time.time()
    ...
    plt.title('CVRP Best Route Found by Dijkstra')
    plt.show()
